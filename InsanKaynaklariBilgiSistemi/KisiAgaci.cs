...
namespace InsanKaynaklariBilgiSistemi
{
    public class KisiAgaci
    {
        ...
        public KisiAgaci(IkiliAramaAgacDugumu kok)
        {
            this.kok = kok;
        }
        ...
        private void NotKontrol(IkiliAramaAgacDugumu dugum)
        {
            if (((Kisi)dugum.veri).EgitimBilgisi.DoksanUzeriNot() == true) //gönderilen düğümün notu doksan üzerindeyse kişi adını yazdır
                dugumler += ((Kisi)dugum.veri).Ad + Environment.NewLine;
        }

        public void DoksanUzeriNotAra()
        {
            dugumler = "";
            DoksanUzeriNotAraInt(kok);
        }
        private void DoksanUzeriNotAraInt(IkiliAramaAgacDugumu dugum)
        {
            //Ağaçdaki tüm kişileri gezerek not bilgilerini kontrol et
            if (dugum == null)
                return;
            NotKontrol(dugum); //kontrol işlemine git
            DoksanUzeriNotAraInt(dugum.sol); //kontrol bittiyse sola git
            DoksanUzeriNotAraInt(dugum.sag); //kontrol bittiyse sağa git
        }

        private void DilKontrol(IkiliAramaAgacDugumu dugum)
        {
            if (((Kisi)dugum.veri).YabanciDil.Find(stringX => stringX == "İngilizce") == "İngilizce") //gönderilen düğümün yabancı dili ingilizceyse kişi adını yazdır
                dugumler += ((Kisi)dugum.veri).Ad + Environment.NewLine;
        }
